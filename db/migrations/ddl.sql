...
CREATE TABLE COURSE (
    ...
    Title TEXT CHECK (length(Title) BETWEEN 8 AND 32),
...
CREATE TABLE VALID_TAGS (
    ID BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    Title TEXT UNIQUE NOT NULL CHECK (length(Title) BETWEEN 8 AND 32),
    Created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    Updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE TEXT_LESSON (
    ID BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    Lesson_Bucket_ID BIGINT NOT NULL,
    Title TEXT CHECK (length(Title) BETWEEN 8 AND 32),
    Created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    Updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (Lesson_Bucket_ID) REFERENCES LESSON_BUCKET(ID) ON DELETE CASCADE
);
...
CREATE TABLE TEST_LESSON (
    ID BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    Lesson_ID BIGINT NOT NULL REFERENCES LESSON(ID) ON DELETE CASCADE,
    Title TEXT NULL CHECK (length(Title) BETWEEN 8 AND 32),
    Created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    Updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE LESSON_CHEKCPOINT (
    ID BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    User_ID BIGINT NOT NULL REFERENCES USERTABLE(ID) ON DELETE CASCADE,
    Lesson_ID BIGINT NOT NULL REFERENCES LESSON(ID) ON DELETE CASCADE,
    Created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
